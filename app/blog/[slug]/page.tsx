...
export default function BlogDetailPage() {
  ...
  if (!mounted) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <p className="text-muted-foreground font-semibold">Loading Article...</p>
      </div>
    );
  }
  ...
  if (!blog) {
    return (
      <div className="min-h-screen bg-background py-20 flex flex-col items-center justify-center text-center">
        <h2 className="text-3xl font-black text-foreground mb-2">Article Not Found</h2>
        <p className="text-sm text-muted-foreground mb-6 max-w-sm">The article you are trying to view does not exist or was deleted.</p>
  ...
  // Parse lines to render markdown titles and text blocks simply
  const renderContent = (contentStr: string) => {
    return contentStr.split("\n\n").map((block, idx) => {
      if (block.startsWith("### ")) {
        return (
          <h3 key={idx} className="text-lg md:text-xl font-black text-foreground mt-8 mb-4 border-b border-border/20 pb-2">
            {block.replace("### ", "")}
          </h3>
        );
      } else if (block.startsWith("## ")) {
        return (
          <h2 key={idx} className="text-xl md:text-2xl font-black text-foreground mt-10 mb-4 border-b border-border/20 pb-2">
            {block.replace("## ", "")}
          </h2>
        );
      } else if (block.startsWith("- ")) {
      ...
      <main className="py-12 md:py-20">
        <div className="container-main max-w-4xl mx-auto">
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-2xl p-8 mt-12 text-center backdrop-blur-sm shadow-md">
            ...
            <p className="text-xs text-muted-foreground mb-5 max-w-sm mx-auto">Discuss custom Google/Meta campaign architecture and website CRO layouts with our tech leads.</p>
